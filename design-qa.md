...
本实现已由 Spec-009 取代，以下截图、结论和自动验证仅作为旧布局实现的历史记录，不再代表当前产品行为。
...
# 来源网页响应式布局实施复核（Spec-009）

**最后更新：** 2026-07-25

## 实施结论

- 标题栏已移除电脑、平板和竖版布局切换控件；不再维护布局模式或相关 IPC。
- 已删除 768px/1280px 虚拟宽度与随窗口宽度计算缩放的逻辑。远程网页保持 Chromium 默认 100% 缩放。
- `WebContentsView` 仅按来源窗真实内容宽高设置边界，顶部避开 48px 本地标题栏；窗口拉伸不会销毁或重载网页视图。
- 保留桌面 Chromium User-Agent、原生鼠标/滚轮/键盘交互，以及现有远程网页隔离、导航校验和权限拒绝策略。
- 来源窗继续默认跟随聊天面板；手动拖动后在本次打开期间保持自由定位，关闭后恢复默认跟随，且只持久化有效窗口尺寸。

## 自动验证

- 主工程 TypeScript、Electron TypeScript 通过。
- 全量 Vitest：30 个测试文件通过、1 个跳过；89 项通过、8 项跳过。
- 新增真实内容边界单测，覆盖默认尺寸、拉伸尺寸和极小窗口；旧的虚拟布局缩放测试已移除。

## 运行时验收

已在真实 Electron 窗口中确认：响应式网页会随来源窗真实宽度重排；非响应式网页保留原生横向滚动；鼠标、滚轮、键盘输入和文本选择可用；拉伸期间页面未重载且 `WebContentsView` 未重建。
...
- 原网页标题栏显示缩小、当前百分比与放大控件；搜索结果预览不显示控件。
- 可在 50% 至 200% 之间按 10% 步进调整，百分比按钮可恢复 100%。
- 缩放仅作用于当前 `WebContentsView`，不会重载网页或影响聊天窗口、来源列表与其他来源。
- 在真实 Electron 桌面窗口中完成用户人工验收；缩放功能与文字、语音会话的其余功能均未发现问题。
