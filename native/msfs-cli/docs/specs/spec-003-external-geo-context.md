...
- [x] 输入 MSFS 坐标总是以 `WGS84` 标识；Provider 转换发生在 Geo Cloud 内部，响应不得含未标识坐标系的坐标值。
- [x] CLI 在成功响应添加 `origin: "external_geo_cloud"`，不会宣称地理结果来自 SimConnect；Geo Cloud 的 `_meta` 透传在 `context` 中。
- [x] `detail=full` 且 Nominatim Provider 可用时，`place` 返回 WGS84 坐标、显示名、来源和 ODbL 署名；Provider 不可用时保留现有上下文并在 `_meta.degraded` 标识降级。
- [x] 响应的 `_meta.source_map` 必须标明 `administrative`、`natural`、`game_poi` 等字段域的实际来源；不得覆盖 MSFS 原生字段来源。
...
- [x] 机场 ICAO、跑道、导航台与附近机场通过 MSFS `facilities` 获取；不得调用或分发 OurAirports 作为运行时兜底。
...
- 已完成验收（2026-08-17）：Geo Cloud 的 Nominatim `place`、WGS84/来源元数据、Provider 降级、MSFS `--from aircraft` 端到端流程均通过验证。
- Geo response normalization、Geo Cloud client 和运行中 MSFS `--from aircraft` 端到端回归已完成并通过。
