...
**当前验收状态（2026-08-11）：** P0/P1 回归项、双 daemon 隔离、EFB 恢复、取消与长时间前端状态恢复均已完成验收；P2 桌面体验回归也已完成。本文保留为后续版本变更时的回归矩阵，不代表当前仍有未解决问题。
...
设置选择器和聊天气泡的代码改动已完成，提交前已通过类型检查、单元测试、Renderer ESLint、CSS 格式检查和差异检查；以下用例已在真实 Electron 窗口中完成回归。后续若再次回归失败，应记录窗口尺寸、消息内容和复现步骤。
