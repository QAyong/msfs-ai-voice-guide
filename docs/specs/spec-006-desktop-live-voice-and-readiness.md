...
- 首次启动或服务未配置时，聊天面板显示脱敏后的缺失/无效配置项；点击“打开设置”进入应用内设置页完成配置；密钥不回传 Renderer。
...
- [x] 服务配置缺失或无效时显示启动诊断提示；点击“打开设置”进入应用内设置页，保存后重新检测可恢复启动流程。
...
1. 配置无效时，应用停留在启动诊断状态，不创建 Token 或启动外部连接；用户可从提示进入设置页。
