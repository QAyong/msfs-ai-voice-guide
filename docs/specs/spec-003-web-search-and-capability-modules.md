...
导游助手需要从模型外部获取公开网页信息，包括天气、新闻、活动、规则变化、人文地理、偏门地点、非亚洲地区和英文资料。当前支持豆包搜索 Custom API 与博查 Web Search API，供应商结果都必须经过项目相关性保护层后才能交给 LLM，不能停留在“请求 API 后把原文交给 LLM”。
...
- 通过用户选择的搜索服务商提供通用公开网页外部信息能力；当前支持豆包搜索 Custom API 和博查 Web Search API。
...
豆包搜索响应可能随所用接口形态返回 `WebResults` 或 `GlobalSearchResp.Documents`；博查 Web Search API 返回 Bing 兼容的 `webPages.value`。两者的原始供应商结构只能在 `src/search/providers/`（搜索 Provider 适配层）内解析；LiveKit Tool（实时 Agent 工具）、CLI（命令行入口）和桌面端统一消费项目自己的标准化结构。
...
- `scripts/test-volcengine-search.mjs`：豆包搜索手动真实接口评测脚本；博查可通过 `SEARCH_PROVIDER=bocha` 和对应 Key 使用同一 CLI/冒烟链路验证。
...
- [博查 Web Search API](https://open.bochaai.com/)
