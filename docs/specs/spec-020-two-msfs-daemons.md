...
**日期：** 2026-08-10  
**状态：** 已确认并实现，2026-08-10 完成原生、桌面端和实机回归  
**优先级：** P0
...
- [ ] 应用可以同时启动 `monitor` 和 `ai` 两个 daemon，且没有互斥体或 Pipe 冲突。
- [ ] 连接监控、探索和 AI 请求可以同时执行，互不因 Pipe 忙碌而误报对方失败。
- [ ] 用户进行 AI 对话时启动探索，普通 AI 请求不会因为探索而长时间排队。
- [ ] 探索请求失败或超时后，监控仍能继续探测，AI 对话仍能继续。
- [ ] EFB 超时后，下一次普通飞行数据请求仍能在限定时间内完成。
- [ ] 停止或重启 AI daemon 时，监控 daemon 仍能正常检测 MSFS。
- [ ] 停止或重启监控 daemon 时，AI daemon 不被自动关闭。
- [ ] 真实退出 MSFS 时，两个 daemon 都返回真实的不可用结果，前端显示未连接。
- [ ] 应用退出后没有残留 `msfsd.exe`、CLI 进程或占用的 Pipe。
- [ ] 开发态和安装态各完成一次真实 MSFS 2024 回归。

## 与现有规格的关系

本草案如果获批准，将扩展并替代 `Spec-019` 中“一个 daemon、一个 SimConnect 会话”的
拓扑决定。Spec-019 中关于 Pipe 等待、现有 CLI 协议和错误响应的要求继续保留。

在本草案获批准前，不修改 `Spec-019` 的状态和实施记录。
