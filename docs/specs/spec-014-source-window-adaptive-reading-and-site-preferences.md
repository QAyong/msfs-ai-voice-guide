...
来源网页使用一个专用、持久的 Chromium 会话。用户关闭浮窗时会销毁网页文档以释放其渲染进程和页面内存，但不会清理该会话的站点数据。因此网站 Cookie、登录态及用户自行完成的验证可在浮窗关闭、重新打开和应用重启后复用；新打开的网页仍按既有安全生命周期重新加载。
...
- 不由应用业务代码保存、读取或上传网页正文、登录凭据、表单内容、滚动位置、媒体播放状态或完整浏览历史；Chromium 持久会话仅按浏览器标准保留站点数据。
...
1. 来源网页在固定命名的 Chromium 会话分区中运行，所有来源窗口周期复用该分区。
2. 用户点击来源浮窗的关闭按钮、或收起聊天面板时，销毁当前 `WebContentsView` 并关闭浮窗，释放网页渲染进程与页面内存。
3. 再次打开来源时，创建新的安全网页文档，但复用同一会话分区中的 Cookie、Local Storage 与其他标准站点数据；缩放和阅读模式按已保存偏好恢复。
4. 打开另一 URL、返回来源列表、切换移动/桌面模式、加载失败或应用真正退出时，沿用现有受控导航或资源清理逻辑。
5. 验证页面由用户自行处理；应用不得自动点击、提交、规避或代替网站验证。
...
- [ ] 关闭来源浮窗或收起聊天面板后，当前 `WebContentsView` 被销毁并释放页面内存；重新打开时仍复用持久 Chromium 会话中的用户自行完成的验证与站点状态。
- [ ] 重新创建文档不会绕过现有清理、导航白名单和安全检查；打开不同 URL 或切换阅读模式仍使用受控导航。
...
- 2026-07-29：`pnpm verify` 通过（124 项测试通过，8 项既有跳过）。
- 2026-07-29：完整 `pnpm desktop:build` 通过，包含原生按键通话模块重编译、Electron 主进程/Preload/Renderer 打包，以及 MSFS CLI 与 LiveKit 运行时暂存。
