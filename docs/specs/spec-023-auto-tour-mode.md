...
4. 普通聊天正在进行、用户正在语音输入或讲解正在生成/播放时，菜单项按单回合规则显示忙碌或停止状态。
...
Assistant View 需要订阅独立的讲解状态，不把讲解状态复用为 `exploring`：
...
前端可以根据已知状态提前禁用“开启介绍”菜单项，但后端/Agent 的回合协调仍是最终判断：

- 用户正在按住说话或连续对话时，“开启介绍”菜单项不可用；
...
6. **当前前端还没有菜单和讲解 IPC。** 现有 `desktop/renderer/src/main.tsx` 的探索按钮会直接调用 `requestExplore()`，Preload 也只有 `requestExplore`、取消和建议回填，没有 `requestExploreNarration`、讲解状态或自动 user turn 桥接。
