...
- 主题角色先串行生成 3～5 个主题；百科查询、视频查询、推荐问题和 AI 导览介绍四个角色随后并行生成。所有角色复用同一结构化 `requestJson` 调用机制，并显式使用 `thinking: { type: 'disabled' }`；主题、查询词、接续问题和 AI 导览介绍均根据 `locale` 输出，中文介绍最多 100 个汉字，英文介绍最多 60 个单词。介绍角色失败或 6 秒超时返回空值并隐藏，不阻塞其它探索结果；其余结果按 `topicId` 合并并再次经过 Schema 校验。
...
- 中文环境的百科默认值为 `baidu_baike`，英文环境的百科默认值为 `wikipedia`；中文环境的视频默认值为 `bilibili`，英文环境的视频默认值为 `youtube`。首次进入某种语言时使用该语言默认值，用户在该语言下修改后按语言分别保存；切换语言不会把另一种语言的旧选择带过来。
- 设置 UI 将百科呈现为单选、视频呈现为多选，保存时与现有通用偏好一起按语言本地化持久化。旧版本只有一份全局探索来源配置时，若其内容只是另一语言的默认值则按过期配置迁移，否则仅迁移到旧配置所属语言。
...
type ExplorePlan = {
  ...
  introduction?: string; // AI 导览介绍，字段 0..180；中文最多 100 字，英文最多 60 词
};
```
...
探索结果还可携带 `introduction`，它是独立 DeepSeek 导览角色根据最近对话与已生成主题、按 `locale` 生成的自然段；中文最多 100 字，英文最多 60 词，字段为空时来源窗口隐藏该介绍，不影响主题、来源卡片和接续问题。
...
人工验收至少需要验证：中文环境默认百度百科和哔哩哔哩、英文环境默认 Wikipedia 和 YouTube；YouTube 和哔哩哔哩搜索页能在来源窗口中打开；Wikipedia 和百度百科的来源页能在来源窗口中打开；主题描述填充与来源卡片之间的层级清晰；3～5 个词条不重复；以及上述行为在目标网络和 Electron 来源窗口中的可达性。若任一结论不稳定，应保留接口并暂缓对应 Provider，不以模型伪造结果补偿。
