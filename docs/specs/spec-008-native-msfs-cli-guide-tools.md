...
- [ ] Agent 启动时能在开发态与安装态解析 CLI 资源路径，并产生可读的就绪结果。
- [ ] 所有 7 个工具均为 Zod 校验的 `llm.tool()`，不接受原始 CLI 参数、原始 SimVar 名称或 `--unsafe`。
- [ ] `getFlightSnapshot` 使用受控批量读取，返回一致单位、来源和时间戳。
- [ ] `getLocationContext` 能正确透传 Geo Cloud 的字段来源，外部地理失败时不影响基础飞行快照。
- [ ] `getRouteBrief` 与 `getNextWaypoint` 在 EFB route bridge 就绪时读取真实航路；不可用时返回可区分的结构化状态。
- [ ] `getNearbyFacilities` 仅返回 CLI 原生设施结果，并校验半径和设施类型。
- [ ] `getWeatherAndSimTime` 只描述游戏内环境和时间，不将其混同为实时网络天气。
- [ ] `getTrackHistory` 能启动、取消并清理 NDJSON watch，缓存有大小和生命周期上限。
- [ ] CLI 非零退出、stdout 非法 JSON、超时和 `SIM_NOT_READY` 不会使 Agent Worker 崩溃，也不会泄漏原始堆栈或密钥。
- [ ] 默认注册集合只包含 7 个 MSFS 工具与现有 `searchWeb`；不注册任何写操作。
- [ ] 覆盖单元、集成和模拟器未就绪场景的自动化测试。
