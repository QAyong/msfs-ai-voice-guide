...
“通用”页的语音输入分组把“按住说话输入”作为键盘/鼠标侧键设置，而非语音交互模式选择。控件包含常用预设和自定义键盘录入：
...
已完成设置的双页纵向布局、通用偏好保存、受信任设置窗中的遮罩凭据显示/隐藏、`safeStorage` 加密保存、主助手与设置的中英文切换，以及语言切换后新 Agent 会话的回复语言和 STT 默认语言切换。

以下验收项尚未完成，界面不得暗示其已经可用：

- 全局按住说话尚未接入 Windows 原生键盘/鼠标钩子。当前仅应用窗口内的空格键和鼠标按住说话可用；下拉框中的其它键位只是待接线的偏好值。
- 服务页的“保存并重新连接”尚未形成候选配置、Worker 重启、Room 重连与失败回滚链路。当前保存的加密凭据也尚未被注入实际 Agent 配置。
- 诊断 ZIP、日志保留、脱敏、系统保存对话框和原子归档写入均未实现；“导出诊断包”必须保持不可用。
- 来源窗口可信工具栏尚未随 English 切换而完整本地化。
- 全局按键状态机、键位校验/迁移、诊断脱敏与 ZIP 清单的自动化测试，以及 MSFS 前台 Windows 人工验收仍待完成。
