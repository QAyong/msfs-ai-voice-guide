...
已完成候选服务配置、隔离候选 Worker、Room 重连与失败回滚链路：候选会话连接成功后才提交设置并停止旧 Worker。经 Windows `safeStorage` 保存的凭据仅在主进程中注入实际 Agent 配置。每个服务都能在不保存草稿的前提下进行连通性检测；检测具有 10 秒超时、脱敏错误反馈与 2.5 秒限流。

已完成诊断导出：主进程按日维护 main、Worker、对话及工具事件 NDJSON，保留最长 7 天、总量不超过 10 MiB。所有写入与归档前共用脱敏器；设置窗口通过系统保存对话框选择位置，使用流式 ZIP 写入临时文件后原子移动。导出按钮已启用，正常状态显示可点击光标和悬停反馈，只有归档进行中才显示等待状态。
