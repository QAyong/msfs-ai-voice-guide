...
1. 开发者复制 `.env.example` 为本地 `.env`，填入 LiveKit 与已选择 Provider 的凭据。
...
4. 在 `providers` 中接入第一套已选 Provider，并将创建逻辑集中化。
5. 在 `agent` 中按官方当前 SDK 类型实现入口、worker/dispatcher 与语音会话。
6. 进行本地房间人工联调，补充不依赖真实密钥的集成测试。
7. 只有在该流程稳定后，另立 Spec 实现工具调用和模拟器数据模块。

## 相关测试

- `tests/unit/config.test.ts`：环境变量校验、错误脱敏、Provider 选择。
...
- [ADR-002：最小 Provider 装配与延迟抽象](../adr/adr-002-minimal-provider-composition.md)
