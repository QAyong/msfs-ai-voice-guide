...
3. 设置使用真实按钮组成键盘/鼠标设备点选器：键盘直接提供 `Left Alt`、`F8`、`F9`、`Right Ctrl`、`Caps Lock`、`Space` 六个键盘预设，并将 `Esc`、`F1`、`F2`、`F12`、`Q`、`W`、`E`、`R`、`T` 作为进入自定义键位的快捷按钮；鼠标提供位于左侧拇指位的 `X1`（后退）及 `X2`（前进），全局预设合计八个。
...
7. 2026-08-22 已修复键盘点选器中视觉键帽与实际交互不一致的问题：所有展示的可选键均为真实按钮，带有选中、焦点、悬停和按下反馈；`desktop:typecheck`、Renderer ESLint、Prettier 和完整 Vitest 套件均通过（60 个测试文件通过、1 个跳过；226 个测试通过、8 个跳过）。
