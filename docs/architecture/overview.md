...
**阶段：** 第一版语音闭环、网络搜索工具、Electron 桌面基础界面和本地麦克风音量反馈已实现；LiveKit Room 接入与安装包待完成
...
桌面端采用两个视觉窗口：常驻的语音聊天窗口，以及按需出现的伴随来源浏览窗。Electron 主进程已经负责透明置顶窗口、多显示器定位、来源窗伴随定位和远程网页安全策略；可信 Renderer 已能创建并复用 LiveKit 本地麦克风音轨，通过同一音轨的 Web Audio 分析结果提供实时音量反馈。短期 Token 获取、Room 连接、音轨发布、Agent 音频订阅和 Agent 生命周期仍待接入。
...
| 桌面模块                            | 职责                                                                                           | 安全边界                                                   |
| ----------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| Electron Main Process（主进程）     | 创建/关闭窗口、置顶、多显示器定位、来源窗伴随定位；后续接入全局快捷键、持久化和 Agent 生命周期 | 校验所有 IPC 发送方与 URL；不把密钥发送给远程网页          |
| Assistant BrowserWindow（助手窗口） | 独立拖动把手、头像点击展开、聊天气泡、本地麦克风轨道、实时音量反馈、来源卡片                   | 只加载应用本地可信 UI；不持有 LiveKit API Secret           |
...
- Renderer 不得持有 LiveKit API Key 或 API Secret。正式 Room 接入应由可信后端签发权限最小、有效期短的参与者 Token。
...
- 每名用户使用独立 LiveKit 房间，Agent 只服务该房间上下文。桌面客户端目前只有本地音轨，房间命名、短期 Token、音轨发布与远端音频订阅需在端到端联调前完成。
...
| LiveKit JavaScript SDK     | 桌面麦克风轨道和后续 Room 客户端 | `desktop/renderer/`         | 本地轨道已实现；Room 待接入 |
...
| Electron                   | Windows 桌面壳、透明置顶窗口     | `desktop/main/`             | 基础实现已完成              |
| React + Vite               | 助手与来源窗口的本地可信 UI      | `desktop/renderer/`         | 基础实现已完成              |
