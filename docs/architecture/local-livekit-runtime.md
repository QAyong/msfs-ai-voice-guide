...
本文描述 Windows 安装包内的单机实时运行方式。开发态由 Electron 默认启动 `resources/livekit/livekit-server.exe`，安装态从应用资源目录启动同一受控运行时；两者都使用动态回环端口、运行时生成凭据和主进程生命周期管理。V2 x64 候选安装包已生成并完成本机安装验证，跨机器干净环境回归仍待完成。
...
6. 待完成：在另一台无开发依赖的干净 Windows 环境完成跨机器验收。
