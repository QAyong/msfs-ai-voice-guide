...
**最后更新：** 2026-08-22
**状态：** 开发态与 Windows x64 安装态均已实现；安装态依赖和子进程自动校验通过，仍需其他干净 Windows 机器回归

本文描述 Windows 安装包内的单机实时运行方式。开发态由 Electron 默认启动 `resources/livekit/livekit-server.exe`，安装态从应用资源目录启动同一受控运行时；两者都使用动态回环端口、运行时生成凭据和主进程生命周期管理。V2 x64 候选安装包已生成并完成当前版本验收，跨机器干净环境回归不纳入当前版本范围。
...
启动、停止、配置保存和失败回滚由主进程串行化处理。停止时不会立即丢弃旧进程引用，而是等待 Worker 或 LiveKit Server 收到退出事件；超时后才执行强制结束，确认旧进程退出前不会启动下一实例。这样可以避免保存配置或快速重试时两个进程同时抢占本机端口。若仍有外部旧进程占用 Worker 健康检查端口 `8098`，诊断会明确显示“本地服务端口已被占用”，不会把它误报为远程 LiveKit 连接失败，也不会自动结束未知进程。
...
| LiveKit Key/Secret | 自动生成本地运行时凭据；外部模式才使用 `.env` 中的连接配置                                                               | 首次初始化生成的每实例值，任何 Renderer 均不可读取 |
...
4. 已完成：V2 安装包携带规则、回环端口、退出清理、Token/dispatch 和安装态 `utilityProcess` 自动校验。
5. 已完成：在当前开发机执行候选安装、服务配置和运行时回归。
6. 跨机器干净 Windows 环境回归作为后续发布工作，不纳入当前版本验收范围。
