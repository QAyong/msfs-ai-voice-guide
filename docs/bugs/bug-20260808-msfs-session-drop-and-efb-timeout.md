...
**发现日期：** 2026-08-08  
**状态：** 已解决并完成验收（2026-08-11）。EFB bridge 根因已确认并通过官方重建/部署恢复；双 daemon 隔离、取消、长时间前端状态恢复和普通 `msfs.exe` 请求的超时与退出边界均已完成验收。
**优先级：** P0  
...
- [x] 已提供 EFB 的稳定复现、游戏日志和恢复后实机结果；Named Pipe 忙碌等待与假断连边界已纳入验收记录。
...
- [x] 已补充取消和长时间前端状态恢复场景；请求隔离、Pipe 忙碌等待、超时边界和单 daemon 重启均已覆盖。
- [x] 当前 Community bridge 已完成 MSFS 实机回归；正式安装器的完整交互式验收仍应在每个候选版本执行。
- [x] 已完成 [Spec-018](../specs/spec-018-msfs-runtime-stability-and-desktop-consistency.md) 的 P0 验收项。
