...
import { z } from 'zod';
import { setAutopilotInputSchema, type MsfsGuideService } from '../msfs/guide-service.js';
import {
...
export function createMsfsGuideTools(
  ...
  const tools: readonly llm.ToolContextEntry[] = [
    ...
    llm.tool({
      name: 'setAutopilot',
      description:
        '执行受控的 MSFS 自动驾驶操作，包括 AP、FD、HDG、NAV、ALT、VS、FLC 和目标参数。只有用户已经明确要求执行时才调用；如果用户只是询问能否设置、想了解方案或缺少关键参数，不要调用。工具会在内部检查当前飞机能力，只使用白名单事件，执行后读取实际状态；能力状态为 unknown 或 unsupported 时不会写入模拟器。只有结果明确为 supported 时才能告诉用户“支持”；unknown 必须解释为“目前无法确认”，不能说成支持，也不能擅自断言为不支持。如果结果说明某项设置发送后读回没有生效，必须说明尚未确认该功能，停止继续尝试，不要笼统说成自动驾驶操作失败。不要用 AUTOPILOT AVAILABLE 推断 FD 或 FLC 支持。',
